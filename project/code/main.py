...
from metrics import metrics_dictionary
...
def network_result(model_class, strategy, params, data):
    model = model_class(strategy, input_size=params['input_size'], horizons=params['horizons'], inits=params['inits'])
...
if __name__ == '__main__':
    ...
    df.to_csv('../data/australia_preprocessed.csv')

    for central in ['BOCORWF1','MACARTH1','STARHLWF']: # ['BOCORWF1','MACARTH1','STARHLWF']
        serie = df[central]
        experiment_id = mlflow.create_experiment(central)

        walkforward_validation, _ = timeseries_split(serie, groupby='month', train_expand=False, test_size=1, holdout_size=1)
        ...
        for folder, (train, test) in enumerate(walkforward_validation, start=1):
            ...
            for h in range(1, HORIZONS+1):
                ...
                metrics = metrics_dictionary(pred.values[:,h-1], test)
            ...
            scaled_train, scaled_test, scaler = scaling(train, test, feature_range=(0,1))
            ...
            for input_size in [4,12]:   # [4,12]
                for hidden_layers in [[10], [20,10]]:  # [[10], [20,10]]
                    ...
                    X_train, Y_train = supervised_patterns(scaled_train, params['input_size'], params['timelag'], params['horizons'], dropnan=True)
                    X_test = supervised_patterns(scaled_test, params['input_size'], params['timelag'], dropnan=False)
