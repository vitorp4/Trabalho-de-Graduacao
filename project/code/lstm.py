        ...
        if self.strategy == 'direct':
            for h in range(1, self.horizons+1):
                ...
                if len(hidden_layers) == 1:
                    ...
                    model.add(LSTM_layer(neurons, activation=activation, input_shape=(self.input_size, 1)))
                else:
                    for i, neurons in enumerate(hidden_layers):
                        if i == 0:
                            model.add(LSTM_layer(neurons, activation=activation, return_sequences=True, input_shape=(self.input_size, 1)))
                        else:
                            model.add(LSTM_layer(neurons, activation=activation))
                model.add(Dense(1))
    ...
    def predict(self, X, scaler=None, index=None):
        ...
        if self.strategy == 'direct':
            ...
            for h in range(1, self.horizons+1):
                ...
                pred[f't+{h}'] = inverse_scaling(pred_h, scaler, h)
            ...
            for h in range(1, self.horizons+1):
                pred_h = model.predict(X)
                pred[f't+{h}'] = inverse_scaling(pred_h, scaler, h)
                pred_h = pred_h.reshape((pred_h.shape[0], pred_h.shape[1], 1))
